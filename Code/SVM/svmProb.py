...
def main():
    ...
    OUTPUT_FILE  = "../../output/svm_track1_K_" + KERNEL + "_C_" + str(C_CONST) + "_Gamma_" + str(GAMMA) + ".csv"
    ...
    clf  = svm.SVC(C=C_CONST, kernel=KERNEL, gamma=GAMMA, probability=True)
    clf.fit(trainX, trainY)

    t2 = time.time()
    print "...training costs " + str(t2 - t1) + " second, start predicting test data..."

    data = clf.predict_proba(testX)
